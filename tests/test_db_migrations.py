...
def test_registry_is_contiguous_and_matches_immutable_manifest(tmp_path: Path):
    ...
    assert [entry["version"] for entry in manifest["migrations"]] == [1, 2, 3, 4, 5, 6]
    assert len({entry["checksum"] for entry in manifest["migrations"]}) == 6
...
def test_v5_to_v6_seeds_definition_history_without_forging_occurrences(
    ...
    assert run_migrations(database._conn, database._migration_steps()) == 6
    migration_v6.validate(database._conn)
...
def test_historical_fixtures_upgrade_across_all_missing_versions(
    ...
    try:
        ...
        history = database.get_prompt_override_version(
            "global", "", job.pipeline, "04_smart", 1
        )
...
def test_live_shape_legacy_glossary_backup_is_preserved_byte_for_byte(
    ...
    try:
        ...
        reopened.close()
    backup = tmp_path / "migration-backups" / "legacy-preserve.pre-v1-to-v6.db"
...
def test_repeated_init_keeps_ledger_and_backup_stable(tmp_path: Path):
    ...
    backup = tmp_path / "migration-backups" / "repeat.pre-v1-to-v6.db"
...
def test_conflicting_ledger_row_is_detected_before_commit_and_rolls_back_v7(
    tmp_path: Path,
):
    ...
    payload = "future-v7-ledger-conflict-fixture"

    def apply_v7(connection: sqlite3.Connection) -> None:
        connection.execute("CREATE TABLE future_v7(value TEXT NOT NULL)")
        connection.execute("INSERT INTO future_v7 VALUES ('must-rollback')")
        connection.execute(
            "INSERT INTO schema_migrations(version, name, checksum, applied_at) "
            "VALUES (7, 'conflict', ?, 'now')",
            ("0" * 64,),
        )

    migrations = (
        *database._migration_steps(),
        Migration(7, "future-v7", payload, apply_v7),
    )
    manifest = load_manifest()
    manifest["current_version"] = 7
    ...
    manifest_path = tmp_path / "manifest-v7.json"
    manifest_path.write_text(
        json.dumps(manifest, ensure_ascii=False), encoding="utf-8"
    )

    with pytest.raises(MigrationHistoryError, match="v7.*不一致"):
        run_migrations(
            database._conn,
            migrations,
    ...
    try:
        ...
        assert database._conn.execute(
            "SELECT 1 FROM sqlite_master WHERE type='table' AND name='future_v7'"
        ).fetchone() is None
        assert database._conn.execute(
...
def test_pending_v6_to_v8_chain_rolls_back_every_step_when_v8_fails(
    tmp_path: Path,
):
    ...
    def apply_v7(connection: sqlite3.Connection) -> None:
        connection.execute("CREATE TABLE future_v7(value TEXT NOT NULL)")
        connection.execute("INSERT INTO future_v7 VALUES ('v7')")

    def apply_v8(connection: sqlite3.Connection) -> None:
        connection.execute("CREATE TABLE future_v8(value TEXT NOT NULL)")
        connection.execute("INSERT INTO future_v8 VALUES ('v8')")

    schema_v7 = (
        migration_v6.CURRENT_SCHEMA_SQL
        + "\nCREATE TABLE future_v7(value TEXT NOT NULL);\n"
    )
    schema_v8 = schema_v7 + "\nCREATE TABLE future_v8(value TEXT NOT NULL);\n"

    def validate_v7(connection: sqlite3.Connection) -> None:
        migration_v1._validate_complete_schema(connection, schema_v7)

    def validate_v8(connection: sqlite3.Connection) -> None:
        migration_v1._validate_complete_schema(connection, schema_v8)

    payload_v7 = "synthetic-atomic-v7"
    payload_v8 = "synthetic-atomic-v8"
    migrations = (
        *database._migration_steps(),
        Migration(7, "synthetic-v7", payload_v7, apply_v7, validate_v7),
        Migration(8, "synthetic-v8", payload_v8, apply_v8, validate_v8),
    )
    manifest = load_manifest()
    manifest["current_version"] = 8
    manifest["migrations"].extend(
        [
            {
                "version": 7,
                "name": "synthetic-v7",
                "checksum": hashlib.sha256(payload_v7.encode()).hexdigest(),
            },
            {
                "version": 8,
                "name": "synthetic-v8",
                "checksum": hashlib.sha256(payload_v8.encode()).hexdigest(),
            },
        ]
    )
    ...
    def fail_v8(version: int, _connection: sqlite3.Connection) -> None:
        if version == 8:
            raise RuntimeError("v8 后段故障")

    with pytest.raises(MigrationExecutionError, match="回滚到 v6"):
        run_migrations(
            database._conn,
            migrations,
            manifest_path=manifest_path,
            fault_injector=fail_v8,
        )

    assert database.schema_version() == 6
    assert database._conn.execute(
        "SELECT name FROM sqlite_master WHERE name IN ('future_v7', 'future_v8')"
    ).fetchall() == []
    assert [
        row[0]
        for row in database._conn.execute(
    ...
    try:
        assert database._conn.execute(
            "SELECT value FROM future_v7"
        ).fetchone()[0] == "v7"
        assert database._conn.execute(
            "SELECT value FROM future_v8"
        ).fetchone()[0] == "v8"
        assert [
            row[0]
            for row in database._conn.execute(
...
def test_future_complete_validator_rejects_same_name_trigger_or_view_sql_tamper(
    ...
    expected_schema = migration_v6.CURRENT_SCHEMA_SQL + "\n" + future_objects
...
def test_complete_validator_keeps_normal_fts_writes_working(tmp_path: Path):
    ...
    try:
        database._conn.execute(
            ...
            "VALUES ('job-fts', 'article', 'note', 'col', 'general', "
            "'正常标题', '正常写入全文检索')"
        )
        database._conn.execute(
            ...
            "('chunk-fts', 'job-fts', 'note', 'article', 'col', 'general', "
            "'正常标题', '章节', '正常写入分块检索', '{}')"
        )
        database._conn.commit()
        migration_v6.validate(database._conn)
...
def test_two_processes_serialize_backup_and_migration_from_v1(tmp_path: Path):
    ...
    assert [backup.name for backup in backups] == ["concurrent.pre-v1-to-v6.db"]
