...
from scripts.ci_pyproject_change import has_project_version_change, has_relevant_change
...
def test_only_project_version_change_is_ignored() -> None:
    ...
    assert has_project_version_change(BASE, head) is True


def test_non_version_change_does_not_claim_version_change() -> None:
    head = BASE.replace('"redis>=8,<9"', '"redis>=8,<10"')

    assert has_project_version_change(BASE, head) is False
...
def test_version_mode_reports_version_change(monkeypatch, capsys) -> None:
    contents = iter(
        [BASE, BASE.replace('version = "1.5.6"', 'version = "1.5.7"')]
    )
    monkeypatch.setattr(ci_pyproject_change, "git_file", lambda _revision: next(contents))

    assert ci_pyproject_change.main(
        ["ci_pyproject_change.py", "--kind", "version", "base", "head"]
    ) == 0
    assert capsys.readouterr().out == "true\n"
