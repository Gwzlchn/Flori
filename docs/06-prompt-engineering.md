...
Document 的 Prompt override 增加可空 `document_kind` 维度。空 kind 是全部 Document 共用基线，
`research_paper|article|whitepaper|...` 是体裁专属覆盖；解析优先级固定为
`common global < common domain < kind global < kind domain`。该维度只调整翻译、笔记和评审语气/重点，
不能改变 adapter、Document schema、locator 或证据门。旧 paper/article namespace 由 v7 原子迁到相应 kind，
运行时不保留双读。
...
Video、Document、Audio 的概念步在 validate、input hash 和 execute 之间共用同一份来源快照:

- video 和 audio 只读最新版本化智能笔记。
- Document 按最新智能笔记 → `output/translation.json` → `intermediate/document.json` 选择结构化来源，禁止回退原文 Markdown。
...
三条顶层 pipeline 的 `primary/fallback/text_fallback` tier 不按 provider/model 去重；相同配置的相邻 tier 仍保留独立尝试，因此调用次数、retry、usage、AI log 和请求 payload 都不变。
...
domain: "ml" 的 Profile
  ...
  → Collection "LLM 学习" 下的 arXiv 论文 → Document `05_smart` 用它
...
domain "ml":
  处理 B站视频时评审产出 "Attention" "Query/Key/Value" 等 key_terms → 采纳进 ml glossary
    ...
    → 处理同 domain 下的 arXiv 论文（Document `05_smart`）时，这些概念已在术语表中
