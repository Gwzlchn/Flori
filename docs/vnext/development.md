...
每个产品 WP 以批准的 base commit 为基线，在收口报告中列出手写生产代码净增、测试代码净增和产品模块数量变化。这里只移动代码或拆分文件不算降低复杂度；新增模块必须对应新的行为边界，而不是规避行数门。

生产模块达到 280 个非空行时必须预警，并在 deletion pass 中说明删除或按行为边界拆分的结果；超过 300 个非空行仍然停线。超过 1,000 行的集成测试文件在下一次因产品行为而修改时按行为拆分，不为整理数字单独发起大规模重构。

WP12 和 WP13 的执行包必须写明预计新增量、预计删除量和模块数量预算，收口时报告实际值并完成 deletion pass。预算不能仅靠增加文件数满足。
