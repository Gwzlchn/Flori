...
| `jobs` | `id` PK、`source_id` FK CASCADE、`pipeline_revision_id` FK、`trigger`、`rerun_of_job_id` FK SET NULL、`rerun_from_task_key`、`state`、`prompt_snapshot_id` UNIQUE、`prompt_snapshot_sha256`、`prompt_snapshot_json`、`inputs_json`、`request_key` UNIQUE、`request_sha256`、`created_at_ms`、`started_at_ms`、`finished_at_ms`、`error_code`、`error_message` |
