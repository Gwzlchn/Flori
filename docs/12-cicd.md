...
- `unit-normal` / `unit-worker`：push / PR 到 main 触发；普通 job 在全量 pytest collection 前做确定性混合 LPT 预分 14 shard。轻文件保持完整，未知新文件用已有文件中位数估重；超过单组平均历史负载的巨型文件先局部真实 collection，再把全部实际 nodeid 分散到各组。新增、删除和参数化 nodeid 随当次 collection 进入计划，各组调度项并集完整且互不重叠；worker / step / media job 拆 1 shard，
