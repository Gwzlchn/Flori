...
下载步凭证由中心分发:worker 认领 01_download 时经 transport 领取(redis 镜像/runner API,docs/03 §1.7.1),job 侧载凭证仅同机场景。

## 7. 限流

`POST /api/jobs` 与 `POST /api/jobs/upload` 共用 Redis 固定窗口额度。自定义 `APIRoute` 在
FastAPI 解析 JSON/multipart 前完成认证、额度消费，并按 body 外已知条件检查 Worker 能力，因此超限
或静态 pipeline 无可执行 Worker 的上传不会读取大 body；domain/style 等表单条件在 endpoint 持久化前
再次检查。计数 key 只包含 operation 与 principal 的不可逆指纹；Lua 原子完成
`INCR`、首次 `EXPIRE` 和异常 TTL 修复，保证多个 API 副本共用同一窗口。

路由进入 endpoint 后、任何持久化前再次读取 Worker 快照，缩短预检与提交之间的竞态窗口。
Redis 或配置读取异常返回 `503 unavailable`；Document/Audio 无 pipeline 能力覆盖返回
`503 no_workers`；Video 可先进入 `pending`；超限返回 `429 rate_limited` 和 `Retry-After`。详细契约见
`docs/03-contracts.md` §1.1 与 §5。

Video 是例外的显式 manifest 投递：`POST /api/jobs` 只接受 `content_type=video + parts[1..128]`，旧顶层
URL 和 Video upload 直接 `422`。API 在一次提交中写根 `job.json`、每个 Part manifest、`job_parts` 与
完整 scoped step 集；没有 Worker 时仍可返回 `201 pending`。详情、日志、AI 日志、产物和 rerun 都显式
区分 Job/Part scope，不提供按裸步骤名猜 Part 的兼容路径。
