...
1. **接入门禁**：无 cached worker token 的新 worker 持短期 bootstrap token（`POST /api/workers/registration-token` 铸发）调 `POST /api/runner/register`，服务端签发长期 per-worker token（仅返回一次）并单写 Redis liveness + DB 行。已缓存 per-worker token 的 worker 重启后调 `POST /api/runner/resume`，不再依赖 bootstrap token。
...
5. **心跳/下线**：`/heartbeat` 刷新存活、回发中心运行配置，并上报当前并发与已应用配置版本；`/offline` 主动下线；per-worker token 随 worker 删除即吊销。
