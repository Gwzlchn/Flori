...
> 状态：已采纳，部分被 [ADR-0012](0012-case-evidence-authoritative-sources.md) 和 [ADR-0015](0015-unified-document-family.md) 改写。本文的评审闭环仍有效；内容族、步骤名和来源契约以 ADR-0015 为准。

## 背景

三条内容流水线 `video / document / audio` 的末尾分别由 `12_review / 08_review / 05_review` 评审。Document 内的论文、文章和白皮书共享运行步骤，以 `document_kind` 选择体裁 profile。评审对照完整智能笔记与主来源，产出 `review.json`：各内容类型的整数评分维度 + `overall` + `key_terms` + `missing_concepts` + `top3_improvements` + `issues`，并版本化落 `output/versions/review_*.json` 与笔记版本 1:1 配对（`note_file`）。
...
- **结构化 `issues[]`（模型产出，服务端校验）**：`type ∈ consistency | missing_in_source | missing_external | traceability`、`severity`、`dimension`、`claim` 与一句话说明。`supported` 必须提供 `{source, quote}`，且 quote 逐字命中本次评审来源；否则必须标 `insufficient` 并给原因。
- **两个服务端派生布尔（权威，不信模型；刻意不引入 verdict 枚举/多重状态字段，避免重叠）**：
  - `review_reliable`：只接受严格 JSON、non-bool 的 1–5 整数分、完整输入、明确完成的 provider finish reason，以及通过或不适用的 citation 校验。提取抢救、字段缺失、截断、未知结束原因或 citation 不可验证任一命中即 false。**替代静默中性分**——不可信评审永不算「干净通过」、不向 glossary 提供候选术语。
...
- **accuracy 口径反转**：已带源（素材内 OCR 标注 或受控取证 `[E#]`）的「超出转写」数据**不再扣分**；扣分只落「无源裸数字」或「引用与来源对不上」。
- **key_terms 采集受可信度护栏**：仅 schema v2 且 `review_reliable=true` 的评审可作为 glossary fallback；旧版或不可靠评审完全不采集。
...
- **可靠性基线**：解析失败不再回填中性 3.0 分；三条 pipeline 的评审保存完整 `review_input.md` 与来源 hash/bytes/chars，超过防御上限明确失败，不静默裁剪。API 和 UI 将结果投影为 `reliable`、`unreliable`、`legacy_unverified` 三态。
