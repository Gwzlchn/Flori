...
每条内容流水线（video/paper/article/audio）末尾都有一个质量评审步（分别为 `12_review` / `06_review` / `06_review` / `05_review`，pool=ai）。它对照完整智能笔记与主来源，产出 `review.json`：各内容类型的整数评分维度 + `overall` + `key_terms` + `missing_concepts` + `top3_improvements` + `issues`，并版本化落 `output/versions/review_*.json` 与笔记版本 1:1 配对（`note_file`）。
...
- **可靠性基线**：解析失败不再回填中性 3.0 分；四类评审保存完整 `review_input.md` 与来源 hash/bytes/chars，超过防御上限明确失败，不静默裁剪。API 和 UI 将结果投影为 `reliable`、`unreliable`、`legacy_unverified` 三态。
