...
Worker 可能运行在无法被外部连入的内网机器上（如公司 GPU 服务器、有 Claude 订阅的办公电脑）。这些机器只能出站访问外网。需要一种统一机制让任意远程 Worker 接入系统。
...
| 场景 | Worker 类型 | 典型能力标签 |
|------|-----------|-------------|
| 公司 GPU 服务器 | gpu | `gpu`, `vision` |
| 有 Claude 订阅的办公电脑 | ai | `vision`, `claude-cli` |
| 云端 CPU 实例 | cpu | `heavy` |
