...
## 附:完成权威与运行投影(manifest-v1)

自 manifest-v1 起,步骤完成的持久权威是中心存储的 step manifest(`docs/03-contracts.md §7`):

- SQLite `job_steps.status` 与 Redis 步状态是运行投影,可由 manifest 对账重建;`job_steps.input_hash` 是遗留列,manifest 时代输入兼容权威在 `compatibility.input_digest`,不做 DB 缓存。
- `.done` marker 在 `STEP_COMPLETION_MODE=dual` 迁移期与 manifest 双写,读端 manifest 优先;切 manifest-only 后 `.done` 停止读写并由迁移工具 cleanup。
- 失败步骤没有 manifest:staging 内的半成品不是业务产物,备份与下游一律忽略。
