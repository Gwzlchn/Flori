...
- **多来源内容摄入**：Video、Document、Audio 三类内容；论文、文章、白皮书等由 Document 体裁表达，来源以 `configs/sources.yaml` 为准
...
- **可扩展**：新增文档体裁只扩展 registry/Prompt profile；新增媒介能力只扩展 Document adapter，不复制流水线
...
| first-pass | 三类流水线与多体裁 Document 摄入、FTS5 Search / Ask / MCP、集合订阅、概念图、评审、手工建卡 SRS、证据型自动卡片与批量采纳、概念掌握度、知识雷达、远程 Worker 网关 |
...
### 场景四：阅读论文
> 拖拽 PDF 上传或投递 arXiv URL → 原生 HTML/PDF 阅读 → Document Model 结构化 → 对齐译文与中文笔记 → 关联已有概念并可跳回原位置
