...
|------|------------------|----------------|
| 主 CI | backend normal 15 分片 + worker 1 分片(均 4 xdist worker)、真依赖 integration 两分组、frontend Vitest、coverage gate、按路径构建候选并在现有门通过后提升镜像；拓扑以 `.github/workflows/ci.yml` 为准 | 是，非纯文档 push / PR |
