...
PUT    /api/prompts/{pipeline}/{step}                    → 存该步 prompt 覆盖(mode=overwrite|new + note;content 纯空白=删除恢复默认)→ active_version
DELETE /api/prompts/{pipeline}/{step}                    → 删该 (scope,domain) 覆盖(恢复默认,清全部版本)
```
...
- `active_version`：当前激活版本号（无覆盖 `null`）；`versions`：该 `(scope,domain)` 全部历史版本元信息（`[{version, note, created_at}]`，`version` 升序，不含 `content`；无覆盖空数组）。
...
请求体 `{scope, domain?, content, mode?, note?}`。`content` 即覆盖内容（替换该步展示的默认 user-prompt 模板；无模板步作 system prompt）。纯空白即视为删除该覆盖（恢复默认，清全部版本）。
...
删除该 `(scope,domain,pipeline,step)` 覆盖（恢复默认，连同其全部版本历史）。无则 no-op。Response `200 {"status":"deleted",...}`。
