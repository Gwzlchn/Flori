...
依赖未来产物的条件能力仍由 scheduler 在运行时重算。没有完整 Worker 覆盖时在写 storage、DB、
collection 和 lifecycle event 前返回 `503 no_workers`；endpoint 在真正持久化前再次检查，scheduler
保留运行时 fail-fast 作为 Worker 瞬时离线竞态的第二道门。

`content_type` 只允许 `video|document|audio`，可显式指定，也可由 API 根据 URL 自动推断。
...
上传进入 pipeline 前会规范化为既有输入契约：视频统一为 `input/source.mp4`，Document HTML 保存为
`input/source.html`，Document PDF 保存为 `input/source.pdf`，音频保留受支持的标准扩展名。
...
```bash
curl -X POST 'http://localhost:8000/api/jobs/upload?content_type=video' \
  -F "file=@video.mp4" \
...
GET  /api/pipelines                    → 流水线只读:{name,key,label,content_types,document_kinds,source_profiles,steps:[{key,label,pool,needs:[key...],is_ai,has_override}]};needs=依赖(YAML needs→内部 depends_on),前端据此画分层 DAG;is_ai=pool=='ai'(可编辑 AI 节点)、has_override=该 (pipeline,step) 已有 prompt 覆盖;模板/'.'前缀/default 不计
```
...
**所见即所改**:`configs/prompts/templates/*.md` 中的 14 份 tracked 文件是 prompt 正文唯一真源。API 展示和 Worker 执行共用同一解析契约；每次解析从一份原始字节同时导出 UTF-8 文本、SHA-256、来源、覆盖版本和文件路径。单次 API 响应复用已解析结果，单个 Worker 步骤实例也缓存解析结果供指纹、AI 审计和实际调用复用。API 与 Worker 是独立进程，不承诺跨请求共享内存快照；job override 由创建任务时固化的正文和版本保证执行可复现。
...
彻底删除 query 指定的 `(scope,domain,pipeline,document_kind,step)` 覆盖及历史；无则 no-op。回内置默认并保留历史使用 `activate {version:null}`。
