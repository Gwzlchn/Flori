...
Prompt version 的合法范围是 SQLite 有符号 64 位正整数 `1..2^63-1`。历史查询 path 和 activate body 的非整数、`0`、负数或大于上界值统一在 API 层返回 `422`，不会进入 SQLite 绑定；合法范围内但不存在的版本仍返回 `404`。当历史最大版本已是 `2^63-1` 时，`mode=new` 返回 `409`，`overwrite` 当前版本仍可用。
...
- `version` 只接受 `1..2^63-1`；非整数或越界 → `422`。
...
- `version` 的数字范围是 `1..2^63-1`；字符串、浮点、布尔或越界整数 → `422`。
