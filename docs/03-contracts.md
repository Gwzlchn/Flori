...
Query 必须携带 `content_type=video|paper|article|audio`；它让 API 能在读取 multipart body 前完成
...
- `media.lang`：文章/论文正文主语言,使用 ISO 639-1 代码(如 `zh`/`en`/`fr`)；无法识别时为 `unknown`。旧任务的 `unknown`/`non-zh` 会在读取详情时从可读原文 best-effort 重新检测,但不改写历史产物。
- `update_available` / `update_from_step`：逐步比对当前 pipeline 定义与该快照 `.done.def_digest`,并比较任务 Prompt 快照与当前激活内容；有差异时标记可创建新版及首个变化步骤。前端只在该字段为真时展示版本升级入口,普通失败续跑与指定步骤重跑不再与版本升级并列。
...
- `prompt_versions`：`{step: version}`，本任务各 AI 步派发时用的 prompt 覆盖**版本号快照**。HTTP 响应中的 `version` 是十进制字符串，避免 SQLite 64 位整数进入 JavaScript 后丢失精度；内部 `job.json.prompt_overrides[step].version` 仍为整数。无覆盖/旧 job 纯字符串形态的步不出现，故常为 `{}`。前端据此展示版本差异,但升级统一走 `POST /api/jobs/{id}/rebuild`,保留当前快照。
...
- **article**：`01_download` → `02_parse_article` → `03_article_sections` → `04_smart_article`(可选,`smart_note`)/`04_translate_article`(条件) → `04_semantic_attestation` → `05_concepts`(必跑) → `06_review`(可选)。
  - `02_parse_article` 检测正文主语言写入 `parsed.json` 的 `lang`(ISO 639-1,如 `zh`/`en`/`fr`;无法识别为 `unknown`);**非中文**正文额外写标记 `intermediate/needs_translation.json`。
