...
> **Document AI 恢复断点**:`05_smart` 的章节、主题、终稿和导读调用通过全部结构与证据闭包校验后,对应 final 审计记录可附加阶段断点。后续执行只有在重新核对 Job、步骤、阶段、全部上游 prompt、模板、schema、图片字节、concrete provider/model/effort、审计身份和原始输出,并按连续尝试链重新运行同一 parser/validator 后才复用。恢复后的语义证据继续绑定审计证明的原始 provider session,不得借用新调用身份。缺失、乱序、冲突或损坏记录一律重新调用 AI。断点仍是失败诊断 AI log 的恢复信息,不是步骤成功 manifest,不能单独授权下游步骤。
...
- 写入：`PUT /api/workers/{id}/config`，body 只允许 `{"concurrency": 1..128}`;空 body 返回 `400`,携带 `pools` / `tags` / `reject_tags` 等未知字段返回 `422`。写 DB `workers.desired_config`（JSON）并单调递增 `cfg_rev`。Response：`{"cfg_rev": N, "desired_config": {"concurrency": N}}`。128是单Worker控制面上限,不是容量承诺;运营方必须按CPU、内存、Provider配额和步骤峰值选择实际值,大规模并发优先横向分散到多个Worker节点。
...
官方 Worker 产品镜像按能力闭包拆分:`flori-worker-cpu` / `flori-worker-gpu` 是 compute profile,不得声明 `ai` 池或绑定 CLI;`flori-worker-ai-{claude,qoder,codex}` 是 ai profile,只允许 `--pools ai`,且运行时 `FLORI_CLI_PROVIDER` 必须与镜像内 `FLORI_IMAGE_CLI_PROVIDER` 完全一致。三种 AI 镜像各只携带自己的 CLI,不携带 ffmpeg、Poppler、Deno、布局模型和完整 `[steps,gpu]` 依赖;仅保留 AI 步骤在调用后构造可验证来源所必需的受控运行依赖。没有 `FLORI_WORKER_IMAGE_PROFILE` 的自建/裸机运行时保留兼容,仍由现有 pool、provider、二进制和凭证门 fail-closed。前端接入向导选择 `ai` 时必须移除 compute pools,选择 compute pool 时必须移除 `ai`。
...
- **document**：`01_download → 02_parse → 03_structure` 后分成 `04_translate(条件)` 阅读分支和
  ...
  - `05_smart` 消费 `document.json + quality.json + source_segments.json`,不因译文存在而换正文或失效指纹。Quality schema 只接受枚举原因码和有界浅层指标,单文件上限 64 KiB；历史 v1 的 `scanned_pdf_ocr_error:<ExceptionName>` 只在后缀满足安全标识符时归一为 `scanned_pdf_ocr_failed`。原始来源按章节顺序唯一分配,单包正文不超过 46 KiB、source alias 不超过 32 个；单段过长按 UTF-8 完整边界拆分,参考文献目录确定性排除并写覆盖回执。单执行包最多附 5 张真实图片,任一边界超限时按来源顺序拆包,同一图片只归入来源的首个 part。章节卡按 concrete provider 的 `document_smart_parallelism` 使用有界滑动窗口,Qoder 和 Claude 默认均为 4、未声明时保守为 4,全局上限为 8；不得一次把全部章节包提交给 executor。任一包耗尽四次确定性校验后,立即停止补交新包并取消尚未开始的 future,只等待窗口内已运行调用完成审计。随后最多三份主题综合、一次完整正文和一次独立论文导读；终稿与导读的可选知识集合会先排除任一来源成员没有非空 `support_text` 的知识组,终稿接收的主题子项也必须让其完整 `knowledge_refs` 属于这一可证明集合；任一成员被排除时整个子项不进入终稿 Prompt,不会拆分或换成其它引用。步骤开始时冻结四份 Prompt 快照,每阶段最终 prompt 不超过 1 MiB,响应不超过 2 MiB,结构或引用闭包失败最多完整重试三次。终稿是唯一 Markdown 起止标记内的原始正文与结构化模型综合标记块,不要求模型生成 metadata JSON；标题、主题覆盖、图片 placement、used knowledge refs 和审计回执均由服务端从冻结输入确定性派生。缺失或重复标记、截断、尾随内容和未知引用全部拒绝。其它阶段仍使用严格 JSON；额外字段错误会有界列出最多16个对象的多余字段与允许字段,其余对象报告数量；非法 JSON 反馈只报告有界 reason 与行列号,不回显或修补原始输出。每次尝试都重新执行相同的 schema、来源、图片和证据闭包,第四次仍不合法则整个步骤不发布。章节卡必须覆盖背景、问题、方法、结果、限制与读图边界,主题层显式恢复前后关联；最终正文不设统一字数、段落或章节数限制。作者正文只能使用已验证知识引用；结构化模型综合由服务端确定性渲染分析、依据、不确定性和最小证据集。导读从跨章节的有界证据 catalog 固定回答背景与问题、解决思路、验证方法、主要结论与阅读边界。
