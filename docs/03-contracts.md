...
> **Document 章节恢复断点**:`05_smart` 的章节调用通过全部结构与证据闭包校验后,对应 final 审计记录可附加 `chapter_checkpoint`。后续执行只有在重新核对 Job、步骤、章节包、基础与实际尝试 prompt、模板、schema、图片字节、concrete provider/model/effort、审计身份和原始输出,并按连续尝试链重新运行同一 parser/validator 后才复用该章节。缺失、乱序、冲突或损坏记录一律重新调用 AI。该字段仍是失败诊断 AI log 的恢复信息,不是步骤成功 manifest,不能单独授权下游步骤。
