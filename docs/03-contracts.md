...
> `job.json` 另有 `prompt_overrides`（白盒 Phase 2，见 §1.14）：`{step: prompt_content}`，job 创建时由 api 从 DB `prompt_overrides` 按 `(pipeline, domain)` 解析注入，worker step_base 优先用作该步默认 user-prompt 模板的替换（无外置模板的步则作 system prompt）。与 `ai_overrides`（provider 覆盖）正交。
...
请求体 `{scope, domain?, content}`。`content` 即覆盖内容（替换该步展示的默认 user-prompt 模板；无模板步作 system prompt）。纯空白即视为删除该覆盖（恢复默认）。
