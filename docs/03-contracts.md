...
GET  /api/pipelines                    → 流水线只读:{name,key,label,content_types,document_kinds,source_profiles,steps:[{key,label,pool,needs:[key...],is_ai,has_override,prompt_locked}]};needs=依赖(YAML needs→内部 depends_on),前端据此画分层 DAG;is_ai=pool=='ai'(可编辑 AI 节点)、has_override=该 (pipeline,step) 已有 prompt 覆盖、prompt_locked=协议 prompt 只读步(见 §1.15);模板/'.'前缀/default 不计
```
