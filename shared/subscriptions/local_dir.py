...
扩展名 → content_type 直接读取 configs/sources.yaml 的 upload_extensions。
...
# 本地扫描与上传 API 共用 content_types.upload_extensions,不再维护第二份扩展名表。
EXT_CONTENT_TYPE: dict[str, str] = {
    str(ext).lower(): content_type
    for content_type, spec in CONTENT_TYPE_SPECS.items()
    for ext in spec.get("upload_extensions") or []
}
